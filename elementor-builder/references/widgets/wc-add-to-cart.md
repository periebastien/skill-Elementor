# `wc-add-to-cart` — Ajouter au panier personnalisé (Pro)
...
## onglet content
- section_product : `product_id`, `show_quantity`:'yes', `quantity`:nb[=1]
- section_button : `button_type`(|info|success|warning|danger), `text`:txt, `size`(xs|sm|md|lg|xl)[=sm], `selected_icon`:icons{value,library}, `icon_align`(row|row-reverse)[=row], `icon_indent`:slider{unit,size}, `button_css_id`:txt
